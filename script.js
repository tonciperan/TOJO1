...
(function(){
  ...
  var descs={0:'Vedro',1:'Pretežno vedro',2:'Djelomično oblačno',3:'Oblačno',45:'Magla',48:'Magla',51:'Rosulja',53:'Rosulja',55:'Kiša',61:'Lagana kiša',63:'Kiša',65:'Jaka kiša',71:'Lagani snijeg',73:'Snijeg',75:'Jaki snijeg',80:'Pljuskovi',81:'Pljuskovi',82:'Olujni pljuskovi',95:'Grmljavina',96:'Grmljavina',99:'Oluja'};
  fetch('https://api.open-meteo.com/v1/forecast?latitude=43.64&longitude=15.93&current=temperature_2m,weather_code,wind_speed_10m&timezone=Europe%2FZagreb')
    .then(function(r){return r.json();})
    .then(function(d){
      var c=d.current;
      var wc=c.weather_code;
      el.innerHTML='<div style="font-size:32px;line-height:1">'+(icons[wc]||'🌡')+'</div>'+
        '<div class="weather-temp">'+Math.round(c.temperature_2m)+'°C</div>'+
        '<div class="weather-info">'+
          '<span class="weather-desc">'+(descs[wc]||'Temperatura')+'</span>'+
          '<span class="weather-wind">Vjetar '+Math.round(c.wind_speed_10m)+' km/h</span>'+
          '<span class="weather-loc">Grebaštica, Dalmacija</span>'+
        '</div>';
    })
    .catch(function(){el.innerHTML='<span class="weather-loading">Prognoza nedostupna</span>';});
})();

// ── WEATHER WIDGET (EN) ───────────────────────────────────────────
(function(){
  var el=document.getElementById('weather-widget-en');
  if(!el)return;
  var icons={0:'☀️',1:'🌤',2:'⛅',3:'☁️',45:'🌫',48:'🌫',51:'🌦',53:'🌦',55:'🌧',61:'🌧',63:'🌧',65:'🌧',71:'❄️',73:'❄️',75:'❄️',80:'🌦',81:'🌧',82:'⛈',85:'🌨',86:'🌨',95:'⛈',96:'⛈',99:'⛈'};
  var descs={0:'Clear sky',1:'Mostly clear',2:'Partly cloudy',3:'Overcast',45:'Fog',48:'Fog',51:'Drizzle',53:'Drizzle',55:'Rain',61:'Light rain',63:'Rain',65:'Heavy rain',71:'Light snow',73:'Snow',75:'Heavy snow',80:'Showers',81:'Showers',82:'Thunderstorm',95:'Thunderstorm',96:'Thunderstorm',99:'Storm'};
  fetch('https://api.open-meteo.com/v1/forecast?latitude=43.64&longitude=15.93&current=temperature_2m,weather_code,wind_speed_10m&timezone=Europe%2FZagreb')
    .then(function(r){return r.json();})
    .then(function(d){
      var c=d.current;
      ...
      el.innerHTML='<div style="font-size:32px;line-height:1">'+(icons[wc]||'🌡')+'</div>'+
        '<div class="weather-temp">'+Math.round(c.temperature_2m)+'°C</div>'+
        '<div class="weather-info">'+
          '<span class="weather-desc">'+(descs[wc]||'Temperature')+'</span>'+
          '<span class="weather-wind">Wind '+Math.round(c.wind_speed_10m)+' km/h</span>'+
          '<span class="weather-loc">Grebaštica, Dalmatia</span>'+
        '</div>';
    })
    .catch(function(){el.innerHTML='<span class="weather-loading">Forecast unavailable</span>';});
})();
